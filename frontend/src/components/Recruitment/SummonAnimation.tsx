...
export const SummonAnimation: React.FC<SummonAnimationProps> = ({
  ...
  const [phase, setPhase] = useState<'intro' | 'reveal' | 'celebration'>('intro');
  const [currentCharacter, setCurrentCharacter] = useState<SummonResult | null>(null);
  ...
  const startAnimation = () => {
    ...
    setTimeout(() => {
      if (currentResult && ['Epic', 'Legendary', 'Mythical'].includes(currentResult.rarity)) {
          ...
          {phase === 'reveal' && (
            ...
            >
              ...
              >
                ...
                <div className="absolute inset-4 bg-black bg-opacity-20 rounded-xl flex items-center justify-center">
                  <div className="text-8xl">
                    {currentCharacter.character.element === 'Fire' ? '🔥' :
                     ...
                     currentCharacter.character.element === 'Light' ? '☀️' :
                     currentCharacter.character.element === 'Darkness' ? '🌙' :
                     '✨'}
                  </div>
                </div>
                    ...
                    <div className="flex">
                      {Array.from({ length: 5 }, (_, i) => (
                        <span
                          key={i}
                          className={`text-lg ${
                            i < ['Common', 'Uncommon', 'Rare', 'Epic', 'Legendary', 'Mythical'].indexOf(currentCharacter.rarity) + 1
                              ? 'text-yellow-400'
                              : 'text-gray-600'
                          }`}
                        >
                ...
                {/* Rare pull effects */}
                {['Epic', 'Legendary', 'Mythical'].includes(currentCharacter.rarity) && (
                  <motion.div
