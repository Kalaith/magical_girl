...
export const RecruitmentStats: React.FC = () => {
  ...
  const stats = useMemo(() => {
    ...
    // Banner statistics
    const bannerStats = history.reduce((stats, record) => {
      ...
      record.results.forEach(result => {
        if (!stats[record.bannerId].charactersObtained.includes(result.characterId)) {
    ...
    const pityActivations = history.filter(record => record.wasGuaranteed).length;
    const featuredPulls = allResults.filter(result => result.wasFeatured).length;
    const guaranteedPulls = allResults.filter(result => result.wasGuaranteed).length;
    ...
    const averageCostPerCharacter = newCharacters > 0 ? totalSpent.friendshipPoints / newCharacters : 0;
  ...
  const getRarityPercentage = (rarity: Rarity): number => {
    const total = Object.values(stats.rarityStats).reduce((sum, count) => sum + count, 0);
    ...
    <div className="space-y-6">
      ...
      <Card className="p-6">
        ...
        <div className="space-y-4">
          {(['Mythical', 'Legendary', 'Epic', 'Rare', 'Uncommon', 'Common'] as Rarity[]).map(rarity => {
      ...
      {/* Banner Performance */}
      <Card className="p-6">
        <h3 className="text-xl font-bold text-white mb-6">🎭 Banner Performance</h3>
        {Object.keys(stats.bannerStats).length === 0 ? (
          ...
          <div className="space-y-4">
            {Object.entries(stats.bannerStats).map(([bannerId, bannerData]) => (
              <div key={bannerId} className="p-4 bg-gray-800 bg-opacity-50 rounded-lg">
                <div className="flex justify-between items-start mb-3">
                  <div>
                    <h4 className="font-semibold text-white">
                      {bannerId.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())}
                    </h4>
                    <div className="text-sm text-gray-400">
                      {bannerData.pullCount} pulls • {bannerData.charactersObtained.length} characters
                    </div>
                  </div>
                  ...
                  {(['Mythical', 'Legendary', 'Epic', 'Rare', 'Uncommon', 'Common'] as Rarity[]).map(rarity => {
                    ...
                    return (
                      <div key={rarity} className="text-center">
                        <div className={`text-sm font-bold ${getRarityColor(rarity)}`}>
                          {count}
                        </div>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        {/* Luckiest Pull */}
        <Card className="p-6">
          <h3 className="text-lg font-bold text-white mb-4">🍀 Luckiest Pull</h3>
          {stats.luckiestPull ? (
            <div className="space-y-2">
              <div className={`text-lg font-bold ${getRarityColor(stats.luckiestPull.rarity as Rarity)}`}>
                {stats.luckiestPull.character}
              </div>
              <div className="text-sm text-gray-400">
                {stats.luckiestPull.rarity} • Pull #{stats.luckiestPull.pullNumber}
              </div>
        ...
        {/* Unlucky Streak */}
        <Card className="p-6">
          <h3 className="text-lg font-bold text-white mb-4">😅 Longest Dry Streak</h3>
          <div className="space-y-2">
            <div className="text-2xl font-bold text-red-400">
        ...
        <Card className="p-6">
          ...
          <div className="space-y-3">
            <div>
              ...
              <div className="text-sm text-gray-400">Average pulls per session</div>
            </div>
            <div>
              <div className="text-lg font-bold text-green-400">
                {stats.averageCostPerCharacter.toLocaleString()}
              </div>
              <div className="text-sm text-gray-400">Avg cost per new character</div>
            </div>
          </div>
        </Card>
        ...
        {/* Spending Summary */}
        <Card className="p-6">
          <h3 className="text-lg font-bold text-white mb-4">💰 Total Spending</h3>
          <div className="space-y-2">
