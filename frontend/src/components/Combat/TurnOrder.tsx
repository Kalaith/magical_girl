  ...
  return (
    <div className={`bg-white border rounded-lg p-4 ${className}`}>
            ...
            >
              ...
              <div className="text-right">
                <div className={`text-sm font-medium ${
                  isAlive ? 'text-gray-800' : 'text-red-600'
                }`}>
                  {isAlive ? 'Ready' : 'Defeated'}
                </div>
                {entry.delayedTurns > 0 && (
                  <div className="text-xs text-orange-600">
                    Delayed ({entry.delayedTurns})
                  </div>
                )}
              </div>
            </div>
      ...
      <div className="mt-4 text-xs text-gray-600">
        <div>Phase: <span className="font-medium">{turnOrder.phase}</span></div>
        <div>Tiebreaker: <span className="font-medium">{turnOrder.speedTiebreaker}</span></div>
      </div>
    </div>
  );
};
