  ...
  return (
    ...
    >
      ...
      <div className="flex items-center space-x-3">
        ...
        `}>
          <span className="text-lg">{getElementIcon(participant.character.element)}</span>
            ...
            <div className="w-full bg-gray-700 rounded-full h-1.5">
              <motion.div
                className={`h-1.5 rounded-full ${
                  getHealthPercentage() > 50 ? 'bg-green-500' :
                  getHealthPercentage() > 25 ? 'bg-yellow-500' : 'bg-red-500'
                }`}
                initial={{ width: '100%' }}
      ...
      <div className="flex items-center justify-between mt-2">
        ...
        <div className="flex space-x-1">
          {participant.statusEffects.slice(0, 4).map((effect, index) => (
            <div
              key={effect.id}
        ...
        <div className="flex items-center space-x-1 text-xs">
          {hasActed && (
            <span className="text-green-400">✓ Acted</span>
          )}
          {isActive && (
