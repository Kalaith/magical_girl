...
const CharacterCard: React.FC<CharacterCardProps> = ({
  ...
  const getElementIcon = (element: string) => {
    const icons: { [key: string]: string } = {
      ...
      'Ice': '❄️', 'Lightning': '⚡', 'Light': '☀️', 'Darkness': '🌙',
      'Nature': '🌿', 'Celestial': '⭐', 'Void': '🌌', 'Crystal': '💎'
    };
  ...
  const getRarityColor = (rarity: string) => {
    switch (rarity) {
      case 'Mythical': return 'from-red-500 to-pink-600';
  ...
  const totalPower = Object.values(girl.stats).reduce((sum, stat) => sum + stat, 0);
  ...
  return (
    ...
    >
      ...
      >
        {/* Rarity Background */}
        <div className={`absolute inset-0 bg-gradient-to-br ${getRarityColor(girl.rarity)} opacity-10 rounded-lg`} />
        ...
        <div className="relative z-10">
            ...
            <div className="flex">
              {Array.from({ length: 5 }, (_, i) => (
                <span
                  key={i}
                  className={`text-sm ${
                    i < ['Common', 'Uncommon', 'Rare', 'Epic', 'Legendary', 'Mythical'].indexOf(girl.rarity) + 1
                      ? 'text-yellow-400'
                      : 'text-gray-600'
                  }`}
                >
          ...
          <div className="grid grid-cols-2 gap-2 text-xs mb-3">
            <div className="flex justify-between">
              ...
              <span className="text-white font-semibold">{girl.stats.power}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-400">Defense:</span>
              <span className="text-white font-semibold">{girl.stats.defense}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-400">Speed:</span>
              <span className="text-white font-semibold">{girl.stats.speed}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-400">Magic:</span>
              <span className="text-white font-semibold">{girl.stats.magic}</span>
            </div>
          </div>
          ...
          <div className="flex justify-between items-center">
            ...
            <span className="text-lg font-bold text-purple-400">{totalPower}</span>
          </div>
        </div>
...
export const TeamSetup: React.FC<TeamSetupProps> = ({
  ...
  const calculateTeamPower = () => {
    ...
    return selectedGirls.reduce((total, girl) => {
      return total + Object.values(girl.stats).reduce((sum, stat) => sum + stat, 0);
    }, 0);
  };
  ...
  const battleTypes: { value: BattleType; label: string; description: string }[] = [
    {
      value: 'Training',
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-4 bg-gray-800 bg-opacity-50 rounded-lg">
              <div>
                ...
                <div className="text-white font-bold">{selectedTeam.length}/{maxTeamSize}</div>
              </div>
              <div>
                <div className="text-gray-400 text-sm">Total Power</div>
                <div className="text-purple-400 font-bold">{calculateTeamPower()}</div>
              </div>
              <div>
                <div className="text-gray-400 text-sm">Elements</div>
                <div className="text-white font-bold">{getTeamElements().length}</div>
              </div>
              <div>
                <div className="text-gray-400 text-sm">Average Level</div>
                <div className="text-white font-bold">
                  {Math.round(getSelectedGirls().reduce((sum, girl) => sum + girl.level, 0) / selectedTeam.length)}
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {getSelectedGirls().map((girl, index) => (
                <div
                  key={girl.id}
                  ...
                  <div className="flex items-center space-x-2 mb-2">
                    <span className="text-lg">
                      {girl.element === 'Fire' ? '🔥' :
                       ...
                       girl.element === 'Earth' ? '🌱' :
                       girl.element === 'Air' ? '💨' :
                       girl.element === 'Light' ? '☀️' :
                       girl.element === 'Darkness' ? '🌙' : '✨'}
                    </span>
                    <div>
                      <div className="text-white font-semibold text-sm">{girl.name}</div>
                      <div className="text-gray-400 text-xs">Lv.{girl.level}</div>
                    </div>
                  </div>
                  <div className="text-xs text-purple-400">
                    Power: {Object.values(girl.stats).reduce((sum, stat) => sum + stat, 0)}
                  </div>
                </div>
              ))}
            </div>
      ...
      <Card className="p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="text-sm text-gray-400">
            {availableGirls.length} available • {selectedTeam.length}/{maxTeamSize} selected
          </div>
        </div>
        ...
        {selectedTeam.length >= maxTeamSize && (
          ...
          >
            ...
            <div className="text-gray-300 text-sm">
              Maximum team size reached. Deselect a character to add a different one.
            </div>
          </motion.div>
        )}
      </Card>
