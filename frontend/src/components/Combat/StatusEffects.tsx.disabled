      ...
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center space-x-2">
          <span className="text-lg">
            {effect.icon || getEffectIcon(effect.category)}
          </span>
          <div>
      ...
      {effect.duration > 0 && effect.maxDuration > 0 && (
        <div className="mb-2">
          <div className="flex justify-between text-xs mb-1">
            ...
            <span
              className={getDurationColor(effect.duration, effect.maxDuration)}
            >
          ...
          <div className="w-full bg-black bg-opacity-30 rounded-full h-1.5">
            <motion.div
              className={`h-1.5 rounded-full ${
                effect.duration > effect.maxDuration * 0.66
                  ? "bg-green-400"
                  : effect.duration > effect.maxDuration * 0.33
                    ? "bg-yellow-400"
                    : "bg-red-400"
              }`}
              initial={{ width: "100%" }}
            ...
            <div key={i} className="text-xs text-gray-300 flex justify-between">
              ...
              <span
                className={`font-semibold ${
                  effectData.operation === "add" && effectData.modification > 0
                    ? "text-green-400"
                    : effectData.operation === "subtract" ||
                        effectData.modification < 0
                      ? "text-red-400"
                      : "text-yellow-400"
                }`}
              >
                {effectData.operation === "add"
                  ? "+"
                  : effectData.operation === "subtract"
                    ? "-"
                    : ""}
...
export const StatusEffects: React.FC<StatusEffectsProps> = ({
      ...
      <Card className="p-4">
        ...
        <div className="text-center py-8">
          ...
          <p className="text-gray-400">
            Select a character to view status effects
          </p>
        </div>
      </Card>
    );
  ...
  const others = statusEffects.filter(
    (effect) => !["Buff", "Debuff"].includes(effect.type),
  );
