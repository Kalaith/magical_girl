...
const TutorialHighlightComponent: React.FC<TutorialHighlightProps> = ({
  highlight,
  children,
}) => {
          ...
          tooltip.position === "top"
            ...
            : tooltip.position === "bottom"
              ...
              : tooltip.position === "left"
                ? "left-full -ml-1.5 border-b border-r"
                : tooltip.position === "right"
                  ? "right-full -mr-1.5 border-t border-l"
                  : "hidden"
        }`}
      />
    </motion.div>
...
export const TutorialOverlay: React.FC = () => {
  ...
  const [contextualHelpData, setContextualHelpData] =
    useState<ContextualHelp | null>(null);
  ...
  useEffect(() => {
    if (
      ...
      tutorialState.settings.autoAdvanceSpeed !== "manual"
    ) {
      const delay =
        tutorialState.settings.autoAdvanceSpeed === "slow"
          ? 5000
          : tutorialState.settings.autoAdvanceSpeed === "normal"
            ? 3000
            : 1500;
  ...
  useEffect(() => {
    if (tutorialState.analytics.mistakeCount > 3) {
      ...
      tutorialState.analytics.completionTime < 30 &&
      tutorialState.analytics.mistakeCount === 0
    ) {
          ...
          {showContextualHelp && contextualHelpData && (
            ...
            >
              ...
              >
                ...
                {contextualHelpData.quickActions &&
                  contextualHelpData.quickActions.length > 0 && (
                    <div className="mb-4">
                      <h4 className="font-semibold text-gray-400 mb-2">
                        Quick Actions:
                      </h4>
                      <div className="space-y-2">
                        {contextualHelpData.quickActions.map(
                          (action, index) => (
                            <button
                              key={index}
                              onClick={() => {
  ...
  return (
    <>
          ...
          >
            ...
            {currentStep.tooltip && (
              <div
                ...
                style={{
                  top:
                    currentStep.tooltip.position === "center" ? "50%" : "auto",
                  left:
                    currentStep.tooltip.position === "center" ? "50%" : "auto",
                  transform:
                    currentStep.tooltip.position === "center"
                      ? "translate(-50%, -50%)"
                      : "none",
                }}
              >
                <TutorialTooltipComponent
                  ...
                  onNext={
                    currentStep.validation?.required ? undefined : nextStep
                  }
                  onPrevious={
      ...
      <div className="fixed bottom-4 right-20 z-50 text-xs text-gray-400 bg-gray-800 px-3 py-2 rounded">
        <div>
          Press <kbd className="bg-gray-700 px-1 rounded">Space</kbd> or{" "}
          <kbd className="bg-gray-700 px-1 rounded">→</kbd> for next
        </div>
        <div>
          Press <kbd className="bg-gray-700 px-1 rounded">←</kbd> for previous
        </div>
        <div>
          Press <kbd className="bg-gray-700 px-1 rounded">Ctrl+H</kbd> for help
        </div>
        <div>
          Press <kbd className="bg-gray-700 px-1 rounded">Esc</kbd> to skip
        </div>
      </div>
    </>
  );
