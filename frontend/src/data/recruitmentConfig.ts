...
export const RECRUITMENT_CONFIG = {
  ...
  banners: [
    {
      ...
      name: 'Eternal Dreams',
      description: 'The standard recruitment banner featuring all available magical girls with balanced rates.',
...

    {
      ...
      description: 'Limited banner featuring powerful elemental magical girls with increased rates!',
      type: 'Limited' as BannerType,
      rarity: 'Premium' as BannerRarity,
      startDate: Date.now(),
      endDate: Date.now() + (14 * 24 * 60 * 60 * 1000), // 14 days
      isActive: true,
      featuredGirls: ['sakura_fire', 'marina_water', 'terra_earth', 'zephyr_air'],
      rates: {
        'Common': 30,
        'Uncommon': 30,
        'Rare': 20,
      ...
      artwork: {
        ...
        effects: ['fire_particles', 'water_ripples', 'earth_crystals', 'wind_swirls'],
        colors: {
          primary: '#EF4444',
          secondary: '#3B82F6',
          accent: '#F59E0B'
...

    {
      ...
      description: 'Special banner for new players with guaranteed high-rarity pulls!',
  ...
  characterPool: [
    // Common characters
    {
      ...
      backstory: 'A cheerful magical girl who believes in the power of friendship and hope.',
      avatar: {
        base: {
          hair: {
            style: 'twin_tails',
    ...
    // Rare character example
    {
      ...
      backstory: 'A powerful fire magical girl with an unbreakable spirit and burning determination.',
      avatar: {
        base: {
          hair: {
            style: 'long_flowing',
...
export const RECRUITMENT_HELPERS = {
  calculatePityBonus: (counter: number, softPityStart: number, rateIncrease: number, maxIncrease: number): number => {
  ...
  calculateAverageCost: (targetRarity: Rarity, rates: GachaRates, cost: number): number => {
    const rate = rates[targetRarity] / 100;
    return cost / rate;
  },

  simulatePulls: (bannerId: string, targetRarity: Rarity, maxPulls: number = 1000): number => {
