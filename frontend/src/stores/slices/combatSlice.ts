...
export interface CombatSlice {
  ...
  // Action System
  queueAction: (participantId: string, action: CombatAction, targets?: string[]) => boolean;
  executeAction: (participantId: string, actionId: string) => Promise<void>;
  executeActionEffect: (participantId: string, effect: CombatEffect, targets: string[]) => Promise<void>;
  cancelAction: (participantId: string) => void;
  getAvailableActions: (participantId: string) => CombatAction[];
  validateAction: (participantId: string, action: CombatAction, targets?: string[]) => boolean;
  ...
  applyEffect: (target: CombatParticipant, effect: CombatEffect, source: string) => void;
  ...
  updateFormation: (formationId: string, formation: Partial<CombatFormation>) => void;
  deleteFormation: (formationId: string) => void;
  getFormationBonuses: (formationId: string, team: CombatParticipant[]) => any[];

  // AI System
  executeAITurn: (participantId: string) => Promise<void>;
  updateAIKnowledge: (participantId: string, event: string, data: any) => void;
  ...
  getElementalEffectiveness: (attackElement: string, targetElement: string) => number;
...
> = (set, get) => ({
  ...
  startBattle: async (type, enemyTeam, playerTeam, environmentId = 'default') => {
    const battleId = `battle_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;

    // Convert MagicalGirls to CombatParticipants
    const playerParticipants: CombatParticipant[] = playerTeam.map((girl, index) => ({
      id: `player_${girl.id}`,
      source: 'player',
      character: girl,
      position: {
      ...
      currentStats: {
        ...
        elementalResistance: {
          'Light': 0, 'Darkness': 0, 'Fire': 0, 'Water': 0,
          'Earth': 0, 'Air': 0, 'Ice': 0, 'Lightning': 0,
          'Nature': 0, 'Celestial': 0, 'Void': 0, 'Crystal': 0
        }
      },
      maxStats: {
        health: girl.stats.endurance * 10,
    ...
    const enemyParticipants: CombatParticipant[] = enemyTeam.map((enemy, index) => ({
      id: `enemy_${enemy.id}`,
      source: 'ai',
      character: enemy,
      position: {
  ...
  endBattle: (battleId, winner, reason = 'Victory') => {
    set(state => {
      ...
      get().recordBattle({
        ...
        result: winner === 'player' ? 'Victory' : winner === 'enemy' ? 'Defeat' : 'Draw',
  ...
  pauseBattle: (battleId) => {
    set(state => ({
      combatSystem: {
        ...
        activeBattle: state.combatSystem.activeBattle?.id === battleId
  ...
  nextTurn: () => {
    set(state => {
      ...
      const nextIndex = (battle.turnOrder.currentIndex + 1) % battle.turnOrder.participants.length;
      ...
      return {
        combatSystem: {
          ...
          activeBattle: {
            ...
            currentTurn: isNewRound ? battle.currentTurn + 1 : battle.currentTurn
          }
        }
      };
    });
    ...
    if (battle) {
      ...
      const aliveEnemies = battle.enemyTeam.filter(p => p.currentStats.health > 0);
      const aliveAllies = battle.playerTeam.filter(p => p.currentStats.health > 0);
  ...
  endTurn: (participantId) => {
    set(state => {
      ...
      const turnEntry = battle.turnOrder.participants.find(p => p.participantId === participantId);
      if (turnEntry) {
  ...
  getCurrentParticipant: () => {
    ...
    const currentEntry = battle.turnOrder.participants[battle.turnOrder.currentIndex];
  ...
  executeAction: async (participantId, actionId) => {
    ...
    const queuedAction = participant.actionQueue.find(qa => qa.action.id === actionId) as ExtendedQueuedAction;
    ...
    const manaCost = action.costs.find(c => c.resource === 'mana')?.amount || 0;
    if (manaCost > participant.currentStats.mana) {
      get().addLogEntry({
        ...
        details: { required: manaCost, available: participant.currentStats.mana }
      });
      return;
    }
        ...
        participants.map(p =>
          p.id === participantId
            ? {
                ...
                actionQueue: p.actionQueue.filter(qa => qa.action.id !== actionId)
              }
            : p
        );
    ...
    get().addLogEntry({
      ...
      details: { participantId, actionId, targets, effects: action.effects.length }
    });
  ...
  executeActionEffect: async (participantId, effect, targets) => {
    ...
    for (const target of targetParticipants) {
      ...
      if (effect.calculation.scalingStats) {
        for (const [stat, multiplier] of Object.entries(effect.calculation.scalingStats)) {
          if (participant.currentStats[stat as keyof typeof participant.currentStats]) {
            effectValue += (participant.currentStats[stat as keyof typeof participant.currentStats] as number) * multiplier;
          }
        }
      }
      ...
      if (effect.elementalTypes && effect.elementalTypes.length > 0) {
        ...
        const effectiveness = get().getElementalEffectiveness(effectElement, targetElement);
  ...
  validateAction: (participantId, action, targets = []) => {
    ...
    // Check cooldowns
    const now = Date.now();
    const lastUsed = (participant.actionQueue.find(qa => qa.action.id === action.id) as ExtendedQueuedAction)?.queueTime || 0;
    if (action.cooldown && (now - lastUsed) < action.cooldown * 1000) return false;
    ...
    if (action.targeting.type === 'Single') {
      ...
      if (teamRestriction) {
        if (teamRestriction.value === 'enemy') {
          const enemyTeam = participant.position.team === 'player' ? battle.enemyTeam : battle.playerTeam;
          const validTargets = targets.filter(targetId =>
            enemyTeam.some(p => p.id === targetId && p.currentStats.health > 0)
          );
          if (validTargets.length === 0) return false;
        } else if (teamRestriction.value === 'ally') {
          const allyTeam = participant.position.team === 'player' ? battle.playerTeam : battle.enemyTeam;
          const validTargets = targets.filter(targetId =>
            allyTeam.some(p => p.id === targetId && p.currentStats.health > 0)
          );
          if (validTargets.length === 0) return false;
        }
      }
    ...
    const restrictiveEffects = participant.statusEffects.filter(e =>
      e.effects.some(eff => eff.stat === 'actions' && eff.operation === 'disable')
    );
  ...
  calculateDamage: (attacker, target, action) => {
    ...
    const baseDamage = action.effects.find(e => e.type === 'Damage')?.calculation.baseValue || 0;
  ...
  calculateHealing: (caster, target, action) => {
    ...
    const baseHealing = action.effects.find(e => e.type === 'Healing')?.calculation.baseValue || 0;
  ...
  applyEffect: (target, effect, source) => {
    set(state => {
      ...
      const updateParticipant = (participants: CombatParticipant[]) =>
        participants.map(p => {
          ...
          switch (effect.type) {
            case 'Damage':
              newStats.health = Math.max(0, newStats.health - effect.calculation.baseValue);
              break;
            case 'Healing':
              newStats.health = Math.min(p.maxStats.health, newStats.health + effect.calculation.baseValue);
              break;
            case 'Mana':
              if (effect.calculation.baseValue > 0) {
                newStats.mana = Math.min(p.maxStats.mana, newStats.mana + effect.calculation.baseValue);
              } else {
                newStats.mana = Math.max(0, newStats.mana + effect.calculation.baseValue);
              }
              break;
            case 'StatModifier':
    ...
    get().addLogEntry({
      turn: get().combatSystem.activeBattle?.currentTurn || 0,
      phase: 'Action',
      type: effect.type === 'Damage' ? 'Damage' : effect.type === 'Healing' ? 'Healing' : 'Status',
      description: `${target.character.name} ${effect.type === 'Damage' ? 'takes' : 'receives'} ${effect.calculation.baseValue} ${effect.type.toLowerCase()}`,
      details: { targetId: target.id, effectType: effect.type, value: effect.calculation.baseValue, source },
      value: effect.type === 'Damage' ? -effect.calculation.baseValue : effect.calculation.baseValue
    });
  },
        ...
        participants.map(p => {
          const updatedEffects = p.statusEffects
            .map(effect => {
              ...
              if (effect.tickInterval && Date.now() - effect.appliedAt >= effect.tickInterval) {
                for (const eff of effect.effects) {
                  if (eff.operation === 'tick') {
                    // Apply tick damage/healing
                    const newStats = { ...p.currentStats };
                    if (eff.stat === 'health') {
                      newStats.health = Math.max(0, Math.min(p.maxStats.health,
  ...
  getFormationBonuses: (formationId, team) => {
    const formation = get().combatSystem.formations.find(f => f.id === formationId);
    if (!formation) return [];
  ...
  executeAITurn: async (participantId) => {
    ...
    switch (aiType) {
      case 'Aggressive':
        ...
        chosenAction = availableActions.filter(a =>
    ...
    if (chosenAction.targeting.type === 'Self') {
      ...
      if (teamRestriction?.value === 'enemy') {
        ...
        if (validTargets.length > 0) {
          targets = [validTargets[Math.floor(Math.random() * validTargets.length)].id];
        }
      } else if (teamRestriction?.value === 'ally') {
        const allyTeam = participant.position.team === 'player' ? battle.playerTeam : battle.enemyTeam;
        ...
        if (validTargets.length > 0) {
          ...
          const injuredAllies = validTargets.filter(p =>
            p.currentStats.health < p.maxStats.health
          );
          const targetPool = injuredAllies.length > 0 ? injuredAllies : validTargets;
          targets = [targetPool[Math.floor(Math.random() * targetPool.length)].id];
        }
      }
    }
    ...
    set(state => {
      ...
      const updateParticipant = (participants: CombatParticipant[]) =>
        participants.map(p => {
          ...
          switch (event) {
            case 'damage_taken':
              updatedKnowledge.threatLevel = Math.min(10,
    ...
    set(state => {
      ...
      return {
        combatSystem: {
          ...
          activeBattle: state.combatSystem.activeBattle && !battleId
  ...
  recordBattle: (record) => {
    ...
    set(state => ({
      combatSystem: {
        ...state.combatSystem,
        combatHistory: [...state.combatSystem.combatHistory, combatRecord].slice(-100)
      }
    }));
  },
  ...
  getCombatAnalytics: () => {
    ...
    return {
      ...
      winRate: history.length > 0 ? (history.filter(r => r.result === 'Victory').length / history.length) * 100 : 0,
      averageTurns: history.length > 0 ? history.reduce((sum, r) => sum + r.turns, 0) / history.length : 0,
      averageDuration: history.length > 0 ? history.reduce((sum, r) => sum + r.duration, 0) / history.length : 0
    };
  },
  ...
  getParticipant: (participantId) => {
    ...
    return [...battle.playerTeam, ...battle.enemyTeam].find(p => p.id === participantId) || null;
  },
          ...
          p.id === participantId
            ? {
                ...
                currentStats: {
                  ...
                  elementalPower: Math.floor(p.currentStats.elementalPower * 1.5),
