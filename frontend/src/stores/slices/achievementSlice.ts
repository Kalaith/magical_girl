...
export const createAchievementSlice: StateCreator<
  AchievementSlice & {
    ...
    player: { achievements: Achievement[]; resources: Record<string, number>; };
    magicalGirls: Array<{ id: string; level: number; stats: Record<string, number>; }>;
    missions: Array<{ id: string; isCompleted: boolean; type: string; }>;
  },
  [],
  [],
  AchievementSlice
> = (set, get) => ({
  ...
  unlockAchievement: (achievementId) => {
    ...
    set((currentState) => {
      ...
      return {
        ...
        recentlyUnlocked: [
          achievement,
          ...currentState.recentlyUnlocked.slice(0, 4),
        ],
      };
    });
    ...
    switch (event.type) {
      ...
      case "achievement_unlocked":
        ...
        if (event.data.count >= 10) {
          state.updateAchievementProgress(
            "achievement_hunter",
            event.data.count,
          );
        }
  ...
  getFilteredAchievements: () => {
    ...
    return achievements.filter((achievement) => {
      // Category filter
      if (
        achievementFilters.category !== "all" &&
        achievement.category !== achievementFilters.category
      ) {
        return false;
      }
      ...
      if (
        achievementFilters.status === "in-progress" &&
      ...
      if (achievementFilters.search) {
        ...
        const searchableText =
          `${achievement.name} ${achievement.description} ${achievement.tags.join(" ")}`.toLowerCase();
  ...
  calculateAchievementStats: () => {
    ...
    const categoryStats = unlockedAchievements.reduce(
      (stats, achievement) => {
    ...
    const nearCompletion = achievements
      .filter(
        (a) =>
          !a.unlocked && a.progress > 0 && a.progress / a.maxProgress >= 0.8,
      )
  ...
  onTrainingCompleted: (_trainingId, score) => {
    ...
    const trainingCompletedCount =
      state.player?.statistics?.trainingSessionsCompleted || 0;
    state.updateAchievementProgress(
      "first_training",
      Math.min(trainingCompletedCount, 1),
    );
    state.updateAchievementProgress(
      "training_adept",
      Math.min(trainingCompletedCount, 10),
    );
    state.updateAchievementProgress(
      "training_master",
      Math.min(trainingCompletedCount, 50),
    );
  ...
  onMissionCompleted: (_missionId, success, score) => {
    ...
    if (success) {
      state.updateAchievementProgress(
        "first_mission",
        Math.min(missionsCompleted, 1),
      );
      state.updateAchievementProgress(
        "mission_hero",
        Math.min(missionsCompleted, 25),
      );
    ...
    const storyMissions =
      state.missions?.filter((m) => m.type === "Story") || [];
    const completedStoryMissions = storyMissions.filter((m) => m.isCompleted);
    if (
      completedStoryMissions.length === storyMissions.length &&
  ...
  onMagicalGirlUnlocked: (_girlId, rarity) => {
    ...
    state.updateAchievementProgress(
      "first_girl",
      Math.min(unlockedGirls.length, 1),
    );
    state.updateAchievementProgress(
      "full_team",
      Math.min(unlockedGirls.length, 5),
    );
